...
import { initializeLspClient, stopLspClient } from './lsp/client'
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const disposable = vscode.commands.registerCommand('sanity.executeGroq', async (groqQuery) => {
    let config: Config
...
function findVariables(node: unknown, found: string[]): string[] {
  if (
    node &&
    typeof node === 'object' &&
    'type' in node &&
  ...
  return Object.keys(node).reduce(
    (acc, key) => findVariables((node as Record<string, unknown>)[key], acc),
    found
  )
}
...
async function readParamsFile(): Promise<Record<string, unknown>> {
  ...
  if (activeFile && activeFile !== '') {
    const pos = activeFile.lastIndexOf('.')
...
async function readParams(variables: string[]): Promise<Record<string, unknown>> {
  ...
  for (const variable of missing) {
    ...
    try {
      ...
      // noop - value stays as string if not valid JSON
    }
